...
public class StreamUtil{
	...
	public static int copy(InputStream input, OutputStream output) throws IOException {
		byte[] buffer = new byte[FwCore.ioBufferSize];
		int count = 0;
		int read;
		while (true) {
			read = input.read(buffer, 0, FwCore.ioBufferSize);
			if (read == -1) {
				break;
			}
	...
	public static int copy(InputStream input, OutputStream output, int byteCount) throws IOException {
		...
		int read;
		while (byteCount > 0) {
			if (byteCount < FwCore.ioBufferSize) {
	...
	public static void copy(InputStream input, Writer output) throws IOException {
		StreamUtil.copy(input, output, FwCore.encoding);
	}
	...
	public static void copy(InputStream input, Writer output, int byteCount) throws IOException {
		StreamUtil.copy(input, output, FwCore.encoding, byteCount);
	}
	/** Copies input stream to writer using buffer and specified encoding. */
	public static void copy(InputStream input, Writer output, String encoding) throws IOException {
		StreamUtil.copy(new InputStreamReader(input, encoding), output);
	}
	...
	public static void copy(InputStream input, Writer output, String encoding, int byteCount) throws IOException {
		StreamUtil.copy(new InputStreamReader(input, encoding), output, byteCount);
	}
	...
	public static int copy(Reader input, Writer output) throws IOException {
		char[] buffer = new char[FwCore.ioBufferSize];
		int count = 0;
		int read;
		while ((read = input.read(buffer, 0, FwCore.ioBufferSize)) >= 0) {
			output.write(buffer, 0, read);
			count += read;
		}
		output.flush();
	...
	public static int copy(Reader input, Writer output, int charCount) throws IOException {
		...
		int read;
		while (charCount > 0) {
			if (charCount < FwCore.ioBufferSize) {
	...
	public static void copy(Reader input, OutputStream output, int charCount) throws IOException {
		StreamUtil.copy(input, output, FwCore.encoding, charCount);
	}
	...
	public static void copy(Reader input, OutputStream output, String encoding, int charCount) throws IOException {
		Writer out = new OutputStreamWriter(output, encoding);
		StreamUtil.copy(input, out, charCount);
		out.flush();
	}
	// ---------------------------------------------------------------- read bytes
	/** Reads all available bytes from InputStream as a byte array. Uses <code>in.available()</code> to determine the size of input stream. This is the fastest method for reading input stream to byte
	 * array, but depends on stream implementation of <code>available()</code>. Buffered internally. */
	public static byte[] readAvailableBytes(InputStream in) throws IOException {
		...
		if (i < l) { throw new IOException("Failed to completely read input stream"); }
		return byteArray;
	}
	...
	public static char[] readChars(InputStream input, int charCount) throws IOException {
		FastCharArrayWriter output = new FastCharArrayWriter();
		StreamUtil.copy(input, output, charCount);
		return output.toCharArray();
	}
