...
public class UnicodeInputStream extends InputStream{
	...
	public UnicodeInputStream(InputStream in, String targetEncoding) {
		internalInputStream = new PushbackInputStream(in, UnicodeInputStream.MAX_BOM_SIZE);
		this.targetEncoding = targetEncoding;
	}
	/** Returns detected UTF encoding or <code>null</code> if no UTF encoding has been detected (i.e. no BOM). If stream is not read yet, it will be {@link #init() initalized} first. */
	...
	protected void init() throws IOException {
		...
		if (targetEncoding == null) {
			// DETECT MODE
			byte bom[] = new byte[UnicodeInputStream.MAX_BOM_SIZE];
			int n = internalInputStream.read(bom, 0, bom.length);
			int unread;
			if ((bom[0] == UnicodeInputStream.BOM_UTF32_BE[0]) && (bom[1] == UnicodeInputStream.BOM_UTF32_BE[1]) && (bom[2] == UnicodeInputStream.BOM_UTF32_BE[2])
					&& (bom[3] == UnicodeInputStream.BOM_UTF32_BE[3])) {
				encoding = "UTF-32BE";
				unread = n - 4;
			} else if ((bom[0] == UnicodeInputStream.BOM_UTF32_LE[0]) && (bom[1] == UnicodeInputStream.BOM_UTF32_LE[1]) && (bom[2] == UnicodeInputStream.BOM_UTF32_LE[2])
					&& (bom[3] == UnicodeInputStream.BOM_UTF32_LE[3])) {
				encoding = "UTF-32LE";
				unread = n - 4;
			} else if ((bom[0] == UnicodeInputStream.BOM_UTF8[0]) && (bom[1] == UnicodeInputStream.BOM_UTF8[1]) && (bom[2] == UnicodeInputStream.BOM_UTF8[2])) {
				encoding = "UTF-8";
				unread = n - 3;
			} else if ((bom[0] == UnicodeInputStream.BOM_UTF16_BE[0]) && (bom[1] == UnicodeInputStream.BOM_UTF16_BE[1])) {
				encoding = "UTF-16BE";
				unread = n - 2;
			} else if ((bom[0] == UnicodeInputStream.BOM_UTF16_LE[0]) && (bom[1] == UnicodeInputStream.BOM_UTF16_LE[1])) {
				encoding = "UTF-16LE";
				unread = n - 2;
			} else {
			...
			BOMSize = UnicodeInputStream.MAX_BOM_SIZE - unread;
			if (unread > 0) {
