...
public class FastCharBuffer implements CharSequence,Appendable{
	...
	/** Creates a new <code>char</code> buffer. The buffer capacity is initially 1024 bytes, though its size increases if necessary. */
	public FastCharBuffer() {
		this(1024);
	}
	/** Creates a new <code>char</code> buffer, with a buffer capacity of the specified size, in bytes.
	...
	private void needNewBuffer(int newCount) {
		if (currentBufferIndex < buffersCount - 1) { // recycling old buffer
			...
			if (currentBuffer == null) {
				...
				newBufferSize = Math.max(currentBuffer.length << 1, newCount - count); // this will give no free additional space
			}
			currentBufferIndex++;
	...
	public FastCharBuffer append(char[] array, int off, int len) {
		int end = off + len;
		if ((off < 0) || (off > array.length) || (len < 0) || (end > array.length) || (end < 0)) { throw new IndexOutOfBoundsException(); }
	...
	/** Returns current index of inner <code>char</code> array chunk. Represents the index of last used inner array chunk. */
	public int index() {
	...
	/** Returns <code>char</code> inner array chunk at given index. May be used for iterating inner chunks in fast manner. */
