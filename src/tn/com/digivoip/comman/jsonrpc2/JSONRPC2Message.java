...
/** The base abstract class for JSON-RPC 2.0 requests, notifications and responses. Provides common methods for parsing (from JSON string) and serialisation (to JSON string) of these three message
 * types.
 * <p>
...
public abstract class JSONRPC2Message implements JSONAware{
	...
	/** Map of non-standard JSON-RPC 2.0 message attributes, {@code null} if none. */
	 ...
	 * <p>
	 * If you are certain about the message type use the dedicated {@link JSONRPC2Request#parse}, {@link JSONRPC2Notification#parse} or {@link JSONRPC2Response#parse} methods. They are more efficient
	 * and provide a more detailed parse error reporting.
	 * <p>
	 * The member order of parsed JSON objects will not be preserved (for efficiency reasons) and the JSON-RPC 2.0 version field must be set to "2.0". To change this behaviour check the optional
	 * {@link #parse(String,boolean,boolean)} method.
	 ...
	 * and provide a more detailed parse error reporting.
	 * @param jsonString A JSON string representing a JSON-RPC 2.0 request, notification or response, UTF-8 encoded. Must not be {@code null}.
	 * @param preserveOrder If {@code true} the member order of JSON objects in parameters and results must be preserved.
	 * @param ignoreVersion If {@code true} the {@code "jsonrpc":"2.0"} version field in the JSON-RPC 2.0 message will not be checked.
	 * @return An instance of {@link JSONRPC2Request}, {@link JSONRPC2Notification} or {@link JSONRPC2Response}.
	 * @throws JSONRPC2ParseException With detailed message if parsing failed. */
	 ...
	 * @param name The attribute name. Must not conflict with the existing "method", "id", "params", "result", "error" and "jsonrpc" attributes reserved by the JSON-RPC 2.0 protocol, else an
	 * {@code IllegalArgumentException} will be thrown. Must not be {@code null} either.
	 * @param value The attribute value. Must be of type String, boolean, number, List, Map or null, else an {@code IllegalArgumentException} will be thrown. */
	public void appendNonStdAttribute(final String name, final Object value) {
		...
		if (name == null ||
				...
				name.equals("result") ||
				name.equals("error") ||
				name.equals("jsonrpc")) { throw new IllegalArgumentException("Non-standard attribute name violation"); }
		// Value check
		if (value != null &&
				...
				!(value instanceof Map)) { throw new IllegalArgumentException("Illegal non-standard attribute value, must map to a valid JSON type"); }
	 ...
	 * @param name The name of the non-standard attribute to retrieve. Must not be {@code null}.
	 * @return The value of the non-standard attribute (may also be {@code null}, {@code null} if not found. */
	 ...
	 * @see #toString
	 * @return The JSON object string representing this JSON-RPC 2.0 message. */
