...
public class JSONRPC2ParseException extends Exception{
	...
	private int					causeType			= JSONRPC2ParseException.PROTOCOL;
	/** The string that could't be parsed. */
	private String				unparsableString	= null;
	
	/** Creates a new parse exception with the specified message. The cause type is set to {@link #PROTOCOL}.
	 * @param message The exception message. */
	public JSONRPC2ParseException(final String message) {
		super(message);
	}
	/** Creates a new parse exception with the specified message and the original string that didn't parse. The cause type is set to {@link #PROTOCOL}.
	 * @param message The exception message.
	 ...
	 * @param message The exception message.
	 * @param causeType The exception cause type, either {@link #PROTOCOL} or {@link #JSON}.
	 * @param unparsableString The unparsable string. */
	public JSONRPC2ParseException(final String message, final int causeType, final String unparsableString) {
		super(message);
		if (causeType != JSONRPC2ParseException.PROTOCOL && causeType != JSONRPC2ParseException.JSON) { throw new IllegalArgumentException("Cause type must be either PROTOCOL or JSON"); }
