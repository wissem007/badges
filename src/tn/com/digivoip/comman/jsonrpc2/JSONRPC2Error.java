 ...
 * <li>{@code message} A string providing a short description of the error. The message should be limited to a concise single sentence.
 * <li>{@code data} Additional information, which may be omitted. Its contents is entirely defined by the application.
 * </ul>
 * <p>
 * Note that the "Error" word in the class name was put there solely to comply with the parlance of the JSON-RPC spec. This class doesn't inherit from {@code java.lang.Error}. It's a regular subclass
 * of {@code java.lang.Exception} and, if thrown, it's to indicate a condition that a reasonable application might want to catch.
 * <p>
...
public class JSONRPC2Error extends Exception{
	 ...
	 * @param err The JSON-RPC 2.0 error to have its data field set. Must not be {@code null}.
	 * @param data Optional error data, must <a href="#map">map</a> to a valid JSON type.
	 ...
	 * @return {@code true} if both objects are instances if this class and their error codes are identical, {@code false} if not. */
	@Override
