 ...
 * <ul>
 * <li>{@code getXXX(param_pos)} for mandatory parameters, where {@code XXX} is the expected parameter type.
 * <li>{@code getOptXXX(param_pos, default_value)} for optional parameters, specifying a default value.
 * </ul>
 * <p>
...
public class PositionalParamsRetriever
	...
	/** Throws a JSON-RPC 2.0 error indicating a missing positional parameter.
	...
	/** Throws a JSON-RPC 2.0 error indicating a positional parameter with unexpected {@code null} value.
	 * @param position The parameter position. Should be non-negative.
	 * @throws JSONRPC2Error Formatted JSON-RPC 2.0 error. */
	...
	/** Throws a JSON-RPC 2.0 error indicating a positional parameter with an unexpected enumerated value.
	 * @param position The parameter position. Should be non-negative.
	...
	/** Creates a JSON-RPC 2.0 error indicating a positional parameter with an unexpected JSON type.
	 * @param position The parameter position. Should be non-negative.
	 ...
	 * @param params The positional parameters list. Must not be {@code null}. */
	...
	/** Returns {@code true} a parameter at the specified position exists, else {@code false}.
	...
	/** Throws a {@code JSONRPC2Error.INVALID_PARAMS} exception if there is no parameter at the specified position.
	 * <p>
	 * You may use this method to fire the proper JSON-RPC 2.0 error on a missing mandatory parameter.
	 * @param position The parameter position, starting with zero for the first.
	...
	/** Throws a {@code JSONRPC2Error.INVALID_PARAMS} exception if there is no parameter at the specified position or its type doesn't map to the specified.
	 * <p>
	 * You may use this method to fire the proper JSON-RPC 2.0 error on a missing or badly-typed mandatory parameter.
	 * @param position The parameter position.
	 * @param clazz The corresponding Java class that the parameter should map to (any one of the return types of the {@code getXXX()} getter methods. Set to {@code Object.class} to allow any type.
	...
	public <T> void ensureParam(final int position,
		...
		if (!clazz.isAssignableFrom(value.getClass())) { throw PositionalParamsRetriever.newUnexpectedParameterTypeException(position); }
	}
	...
	/** Retrieves the specified optional enumerated string parameter. If it doesn't exist the method will return the specified default value.
	 * @param position The parameter position.
	 * @param enumStrings The acceptable string values. Must not be {@code null}.
	 * @param defaultValue The default return value if the parameter doesn't exist. May be {@code null}.
	 * @return The parameter value as a string.
	...
	/** Retrieves the specified optional enumerated parameter (from a JSON string that has a predefined set of possible values). If it doesn't exist the method will return the specified default value.
	 * @param position The parameter position.
	 * @param enumClass An enumeration type with constant names representing the acceptable string values. Must not be {@code null}.
	 * @param defaultValue The default return value if the parameter doesn't exist. May be {@code null}.
	 * @return The matching enumeration constant.
	...
	public List<Object> getList(final int position, final boolean allowNull)
			throws JSONRPC2Error {
		return get(position, List.class, allowNull);
	}
	...
	public List<Object> getOptList(final int position,
			...
			throws JSONRPC2Error {
		return getOpt(position, List.class, allowNull, defaultValue);
	}
	...
	public Map<String, Object> getMap(final int position, final boolean allowNull)
			throws JSONRPC2Error {
		try {
			return get(position, Map.class, allowNull);
		} catch (ClassCastException e) {
			throw PositionalParamsRetriever.newUnexpectedParameterTypeException(position);
		}
	}
	...
	public Map<String, Object> getOptMap(final int position,
			...
			throws JSONRPC2Error {
		try {
			return getOpt(position, Map.class, allowNull, defaultValue);
		} catch (ClassCastException e) {
			throw PositionalParamsRetriever.newUnexpectedParameterTypeException(position);
		}
	}
