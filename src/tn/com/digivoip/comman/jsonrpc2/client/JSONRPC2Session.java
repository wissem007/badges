...
import tn.com.digivoip.comman.jsonrpc2.JSONRPC2Notification;
import tn.com.digivoip.comman.jsonrpc2.JSONRPC2ParseException;
import tn.com.digivoip.comman.jsonrpc2.JSONRPC2Request;
import tn.com.digivoip.comman.jsonrpc2.JSONRPC2Response;
 ...
 * <p>
 * The client-session class has a number of {@link JSONRPC2SessionOptions optional settings}. To change them pass a modified options instance to the {@link #setOptions setOptions()} method.
 * <p>
 ...
 * import tn.com.digivoip.comman.jsonrpc2.client.*;
 * 
 * // The Base package for representing JSON-RPC 2.0 messages
 * import tn.com.digivoip.comman.jsonrpc2.*;
 * 
...
public class JSONRPC2Session{
	...
	private static SSLSocketFactory	trustAllSocketFactory	= JSONRPC2Session.createTrustAllSocketFactory();
	
	/** Creates a new client session to a JSON-RPC 2.0 server at the specified URL. Uses a default {@link JSONRPC2SessionOptions} instance.
	 * @param url The server URL, e.g. "http://jsonrpc.example.com:8080". Must not be {@code null}. */
	public JSONRPC2Session(final URL url) {
		if (!url.getProtocol().equalsIgnoreCase("http") &&
				!url.getProtocol().equalsIgnoreCase("https")) { throw new IllegalArgumentException("The URL protocol must be HTTP or HTTPS"); }
		this.url = url;
	...
	public void setURL(final URL url) {
		if (url == null) { throw new IllegalArgumentException("The server URL must not be null"); }
		this.url = url;
	}
	...
	public void setOptions(final JSONRPC2SessionOptions options) {
		if (options == null) { throw new IllegalArgumentException("The client session options must not be null"); }
	...
	/** Specifies a custom HTTP URL connection configurator. It will be {@link ConnectionConfigurator#configure applied} to each new HTTP connection after the {@link JSONRPC2SessionOptions session
	 * options} are applied and before the connection is established.
	 * <p>
	 * This method may be used to set custom HTTP request headers, timeouts or other properties.
	 * @since 1.5
	 * @param connectionConfigurator A custom HTTP URL connection configurator, {@code null} to remove a previously set one. */
	 ...
	 * @since 1.6
	 * @return The optional inspector for the raw HTTP responses, {@code null} if none is set. */
	public RawResponseInspector getRawResponseInspector() {
		return responseInspector;
	}
	/** Specifies an optional inspector for the raw HTTP responses to JSON-RPC 2.0 requests and notifications. Its {@link RawResponseInspector#inspect inspect} method will be called upon reception of a
	 * HTTP response.
	 * <p>
	 * You can use the {@link RawResponseInspector} interface to retrieve the unparsed response content and headers.
	 * @since 1.6
	 * @param responseInspector An optional inspector for the raw HTTP responses, {@code null} to remove a previously set one. */
	 ...
	 * @return The HTTP cookies, or empty list if none were set by the server or cookies are not {@link JSONRPC2SessionOptions#acceptCookies accepted}. */
	...
	/** Creates and configures a new URL connection to the JSON-RPC 2.0 server endpoint according to the session settings.
	 * @return The URL connection, configured and ready for output (HTTP POST).
	 * @throws JSONRPC2SessionException If the URL connection couldn't be created or configured. */
	private URLConnection createURLConnection()
		...
		if (con instanceof HttpsURLConnection && options.trustsAllCerts()) {
			if (JSONRPC2Session.trustAllSocketFactory == null) { throw new JSONRPC2SessionException("Couldn't obtain trust-all SSL socket factory"); }
			((HttpsURLConnection) con).setSSLSocketFactory(JSONRPC2Session.trustAllSocketFactory);
		}
	 ...
	 * @param con The URL connection. Must be in HTTP POST mode. Must not be {@code null}.
	...
	/** Reads the raw response from an URL connection (after HTTP POST). Invokes the {@link RawResponseInspector} if configured and stores any cookies {@link JSONRPC2SessionOptions#storeCookies if
	 * required}.
	 * @param con The URL connection. It should contain ready data for retrieval. Must not be {@code null}.
	 ...
	 * @param request The JSON-RPC 2.0 request to send. Must not be {@code null}.
	 * @return The JSON-RPC 2.0 response returned by the server.
	 * @throws JSONRPC2SessionException On a network error, unexpected HTTP response content type or invalid JSON-RPC 2.0 response. */
	 ...
	 * @param notification The JSON-RPC 2.0 notification to send. Must not be {@code null}.
