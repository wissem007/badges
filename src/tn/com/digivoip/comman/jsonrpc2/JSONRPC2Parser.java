 ...
 * <p>
 * This class is not thread-safe. A parser instance should not be used by more than one thread unless properly synchronised. Alternatively, you may use the thread-safe {@link JSONRPC2Message#parse}
 ...
 * @version 1.34.3 (2013-02-19) */
public class JSONRPC2Parser{
	
	/** Reusable JSON parser. Not thread-safe! */
	private JSONParser	parser;
	/** If {@code true} the order of the parsed JSON object members must be preserved. */
	private boolean		preserveOrder;
	/** If {@code true} the {@code "jsonrpc":"2.0"} version attribute in the JSON-RPC 2.0 message must be ignored during parsing. */
	private boolean		ignoreVersion;
	/** If {@code true} non-standard JSON-RPC 2.0 message attributes must be parsed too. */
	private boolean		parseNonStdAttributes;
	
	/** Creates a new JSON-RPC 2.0 message parser.
	 * <p>
	 * The member order of parsed JSON objects in parameters and results will not be preserved; strict checking of the 2.0 JSON-RPC version attribute will be enforced; non-standard message attributes
	 * will be ignored. Check the other constructors if you want to specify different behaviour. */
	 ...
	 * @param preserveOrder If {@code true} the member order of JSON objects in parameters and results will be preserved. */
	 ...
	 * @param preserveOrder If {@code true} the member order of JSON objects in parameters and results will be preserved.
	 * @param ignoreVersion If {@code true} the {@code "jsonrpc":"2.0"} version attribute in the JSON-RPC 2.0 message will not be checked. */
	public JSONRPC2Parser(final boolean preserveOrder,
	 ...
	 * @param preserveOrder If {@code true} the member order of JSON objects in parameters and results will be preserved.
	 * @param ignoreVersion If {@code true} the {@code "jsonrpc":"2.0"} version attribute in the JSON-RPC 2.0 message will not be checked.
	 * @param parseNonStdAttributes If {@code true} non-standard attributes found in the JSON-RPC 2.0 messages will be parsed too. */
	public JSONRPC2Parser(final boolean preserveOrder,
			final boolean ignoreVersion,
	...
	/** Parses a JSON object string. Provides the initial parsing of JSON-RPC 2.0 messages. The member order of JSON objects will be preserved if {@link #preserveOrder} is set to {@code true}.
	 * @param jsonString The JSON string to parse. Must not be {@code null}.
	 * @return The parsed JSON object.
	 * @throws JSONRPC2ParseException With detailed message if parsing failed. */
	@SuppressWarnings("unchecked")
	private Map<String, Object> parseJSONObject(final String jsonString)
			throws JSONRPC2ParseException {
		if (jsonString.trim().length() == 0) { throw new JSONRPC2ParseException("Invalid JSON: Empty string",
	...
	/** Ensures the specified parameter is a {@code String} object set to "2.0". This method is intended to check the "jsonrpc" attribute during parsing of JSON-RPC messages.
	 * @param version The version parameter. Must not be {@code null}.
	 * @param jsonString The original JSON string.
	 * @throws JSONRPC2Exception If the parameter is not a string that equals "2.0". */
	 ...
	 * <p>
	 * If a particular message type is expected use the dedicated {@link #parseJSONRPC2Request}, {@link #parseJSONRPC2Notification} and {@link #parseJSONRPC2Response} methods. They are more efficient
	 * and would provide you with more detailed parse error reporting.
	 * @param jsonString A JSON string representing a JSON-RPC 2.0 request, notification or response, UTF-8 encoded. Must not be {@code null}.
	 * @return An instance of {@link JSONRPC2Request}, {@link JSONRPC2Notification} or {@link JSONRPC2Response}.
	 * @throws JSONRPC2ParseException With detailed message if the parsing failed. */
	 ...
	 * @param jsonString The JSON-RPC 2.0 request string, UTF-8 encoded. Must not be {@code null}.
	...
	public JSONRPC2Request parseJSONRPC2Request(final String jsonString)
		...
		if (id != null &&
				...
				!(id instanceof String)) { throw new JSONRPC2ParseException("Invalid JSON-RPC 2.0 request: Identifier not a JSON scalar", jsonString); }
		// Extract params
	 ...
	 * @param jsonString The JSON-RPC 2.0 notification string, UTF-8 encoded. Must not be {@code null}.
	 ...
	 * @param jsonString The JSON-RPC 2.0 response string, UTF-8 encoded. Must not be {@code null}.
	...
	public JSONRPC2Response parseJSONRPC2Response(final String jsonString)
		...
		if (id != null &&
				!(id instanceof Boolean) &&
				!(id instanceof Number) &&
				!(id instanceof String)) { throw new JSONRPC2ParseException("Invalid JSON-RPC 2.0 response: Identifier not a JSON scalar", jsonString); }
	 ...
	 * @param preserveOrder {@code true} to preserve the order of JSON object members, else {@code false}. */
	public void preserveOrder(final boolean preserveOrder) {
		this.preserveOrder = preserveOrder;
	}
	/** Returns {@code true} if the order of JSON object members in parsed JSON-RPC 2.0 messages is preserved, else {@code false}.
	 ...
	 * <p>
	 * You may with to disable strict 2.0 version checking if the parsed JSON-RPC 2.0 messages don't include a version attribute or if you wish to achieve limited compatibility with older JSON-RPC
	 * protocol versions.
	 * @param ignore {@code true} to skip checks of the {@code "jsonrpc":"2.0"} version attribute in parsed JSON-RPC 2.0 messages, else {@code false}. */
	public void ignoreVersion(final boolean ignore) {
		ignoreVersion = ignore;
	}
	/** Returns {@code true} if the {@code "jsonrpc":"2.0"} version attribute in parsed JSON-RPC 2.0 messages is ignored, else {@code false}.
	 * @return {@code true} if the {@code "jsonrpc":"2.0"} version attribute in parsed JSON-RPC 2.0 messages is ignored, else {@code false}. */
	...
	/** Returns {@code true} if non-standard attributes in JSON-RPC 2.0 messages are parsed.
