...
/** Checks whether a string or path matches a given wildcard pattern. Possible patterns allow to match single characters ('?') or any count of characters ('*'). Wildcard characters can be escaped (by
 * an '\'). When matching path, deep tree wildcard also can be used ('**').
 * <p>
...
public class Wildcard{
	...
	/** Checks if two strings are equals or if they {@link #match(String, String)}. Useful for cases when matching a lot of equal strings and speed is important. */
	public static boolean equalsOrMatch(String string, String pattern) {
		if (string.equals(pattern) == true) { return true; }
		return Wildcard.match(string, pattern, 0, 0);
	}
			...
			if (nextIsNotWildcard == false) {
				...
				if (p == '*') {
					...
					for (i = string.length(); i >= sNdx; i--) {
						if (Wildcard.match(string, pattern, i, pNdx) == true) { return true; }
					}
					return false;
				}
			} else {
	...
	public static int matchOne(String src, String[] patterns) {
		for (int i = 0; i < patterns.length; i++) {
			if (Wildcard.match(src, patterns[i]) == true) { return i; }
		}
		return -1;
	}
	...
	public static int matchPathOne(String path, String[] patterns) {
		for (int i = 0; i < patterns.length; i++) {
			if (Wildcard.matchPath(path, patterns[i]) == true) { return i; }
		}
		return -1;
	}
	...
	public static boolean matchPath(String path, String pattern) {
		String[] pathElements = StringUtil.splitc(path, Wildcard.PATH_SEPARATORS);
		String[] patternElements = StringUtil.splitc(pattern, Wildcard.PATH_SEPARATORS);
		return Wildcard.matchTokens(pathElements, patternElements);
	}
	...
	protected static boolean matchTokens(String[] tokens, String[] patterns) {
		...
		while ((patNdxStart <= patNdxEnd) && (tokNdxStart <= tokNdxEnd)) { // find first **
			...
			if (patDir.equals(Wildcard.PATH_MATCH)) {
				break;
			}
			if (!Wildcard.match(tokens[tokNdxStart], patDir)) { return false; }
			patNdxStart++;
			tokNdxStart++;
		}
		if (tokNdxStart > tokNdxEnd) {
			for (int i = patNdxStart; i <= patNdxEnd; i++) { // string is finished
				if (!patterns[i].equals(Wildcard.PATH_MATCH)) { return false; }
			}
			return true;
		}
		if (patNdxStart > patNdxEnd) { return false; // string is not finished, but pattern is
		}
		while ((patNdxStart <= patNdxEnd) && (tokNdxStart <= tokNdxEnd)) { // to the last **
			...
			if (!Wildcard.match(tokens[tokNdxEnd], patDir)) { return false; }
			patNdxEnd--;
			tokNdxEnd--;
		}
		if (tokNdxStart > tokNdxEnd) {
		...
		while ((patNdxStart != patNdxEnd) && (tokNdxStart <= tokNdxEnd)) {
			...
			for (int i = patNdxStart + 1; i <= patNdxEnd; i++) {
				if (patterns[i].equals(Wildcard.PATH_MATCH)) {
					patIdxTmp = i;
					break;
				}
			}
			...
			strLoop: for (int i = 0; i <= strLength - patLength; i++) {
				for (int j = 0; j < patLength; j++) {
					...
					if (!Wildcard.match(subStr, subPat)) {
						continue strLoop;
					}
				}
				ndx = tokNdxStart + i;
