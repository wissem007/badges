...
public class CharUtil{
	...
	public static byte[] toByteArray(char[] carr) throws UnsupportedEncodingException {
		return new String(carr).getBytes(FwCore.encoding);
	}
	...
	public static char[] toCharArray(byte[] barr) throws UnsupportedEncodingException {
		return new String(barr, FwCore.encoding).toCharArray();
	}
	...
	public static int findFirstEqual(char[] source, int index, char[] match) {
		for (int i = index; i < source.length; i++) {
			if (CharUtil.equalsOne(source[i], match) == true) { return i; }
		}
		return -1;
	}
	/** Finds index of the first character in given array the matches any from the given set of characters.
	...
	public static int findFirstDiff(char[] source, int index, char[] match) {
		for (int i = index; i < source.length; i++) {
			if (CharUtil.equalsOne(source[i], match) == false) { return i; }
		}
		return -1;
	}
	/** Finds index of the first character in given array the differs from the given set of characters.
	...
	// ---------------------------------------------------------------- is
	/** Returns <code>true</code> if character is a white space (<= ' '). White space definition is taken from String class (see: <code>trim()</code>). */
	...
	protected static boolean isReserved(char c) {
		return CharUtil.isGenericDelimiter(c) || CharUtil.isReserved(c);
	}
