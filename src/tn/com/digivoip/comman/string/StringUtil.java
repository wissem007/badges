...
import tn.com.digivoip.comman.bean.ObjectUtil;
...
public class StringUtil{
	...
	public static String remove(String string, char ch) {
		...
		if (offset == stringLen) { return string; // no changes
		}
		return new String(result, 0, offset);
	}
	// ---------------------------------------------------------------- miscellaneous
	/** Compares 2 strings. If one of the strings is <code>null</code>, <code>false</code> is returned. if both string are <code>null</code>, <code>true</code> is returned.
	...
	public static boolean isAllEmpty(String... strings) {
		for (String string : strings) {
			if (StringUtil.isEmpty(string) == false) { return false; }
		}
		return true;
	}
	...
	public static boolean isAllBlank(String... strings) {
		for (String string : strings) {
			if (StringUtil.isBlank(string) == false) { return false; }
		}
		return true;
	}
	...
	public static boolean containsOnlyWhitespaces(String string) {
		...
		for (int i = 0; i < size; i++) {
			char c = string.charAt(i);
			if (CharUtil.isWhitespace(c) == false) { return false; }
		}
		return true;
	}
	...
	public static boolean containsOnlyDigits(String string) {
		...
		for (int i = 0; i < size; i++) {
			char c = string.charAt(i);
			if (CharUtil.isDigit(c) == false) { return false; }
		}
		return true;
	}
	/** Returns <code>true</code> if string {@link #containsOnlyDigits(String) contains only digits} or signs plus or minus. */
	public static boolean containsOnlyDigitsAndSigns(String string) {
		int size = string.length();
		for (int i = 0; i < size; i++) {
			char c = string.charAt(i);
			if ((CharUtil.isDigit(c) == false) && (c != '-') && (c != '+')) { return false; }
		}
		return true;
	}
	...
	/** Uncapitalizes a <code>String</code>, changing the first letter to lower case. No other letters are changed.
	...
	/** Utility method to take a string and convert it to normal Java variable name capitalization. This normally means converting the first character from upper case to lower case, but in the
	 * (unusual) special case when there is more than one character and both the first and second characters are upper case, we leave it alone.
	 * <p/>
	...
	// ---------------------------------------------------------------- substring
	/** Returns a new string that is a substring of this string. The substring begins at the specified <code>fromIndex</code> and extends to the character at index <code>toIndex - 1</code>. However,
	 * index values can be negative, and then the real index will be calculated from the strings end. This allows to specify, e.g. <code>substring(1,-1)</code> to cut one character from both ends of
	 * the string. If <code>fromIndex</code> is negative and <code>toIndex</code> is 0, it will return last characters of the string. */
	...
	// ---------------------------------------------------------------- split
	/** Splits a string in several parts (tokens) that are separated by delimiter. Delimiter is <b>always</b> surrounded by two strings! If there is no content between two delimiters, empty string will
	 * be returned for that token. Therefore, the length of the returned array will always be: #delimiters + 1.
	 * <p>
	 * Method is much, much faster then regexp <code>String.split()</code>, and a bit faster then <code>StringTokenizer</code>.
	...
	public static String[] splitc(String src, String d) {
		if ((d.length() == 0) || (src.length() == 0)) { return new String[] { src }; }
	...
	public static String[] splitc(String src, char delimiter) {
		if (src.length() == 0) { return new String[] { StringPool.EMPTY }; }
		char[] srcc = src.toCharArray();
		int maxparts = srcc.length + 1;
		int[] start = new int[maxparts];
		int[] end = new int[maxparts];
	...
	/** Finds first occurrence of a substring in the given source but within limited range [start, end). It is fastest possible code, but still original <code>String.indexOf(String, int)</code> is much
	 * faster (since it uses char[] value directly) and should be used when no range is needed.
	...
	public static int indexOfIgnoreCase(String src, char c, int startIndex, int endIndex) {
		...
		for (int i = startIndex; i < endIndex; i++) {
			if (Character.toLowerCase(src.charAt(i)) == c) { return i; }
		}
		return -1;
	}
	...
	/** Finds first index of a substring in the given source string with ignored case. This seems to be the fastest way doing this, with common string length and content (of course, with no use of
	 * Boyer-Mayer type of algorithms). Other implementations are slower: getting char array first, lower casing the source string, using String.regionMatch etc.
	 * @param src source string for examination
	 * @param subS substring to find
	...
	public static int lastIndexOfIgnoreCase(String src, String sub, int startIndex, int endIndex) {
		int sublen = sub.length();
		int srclen = src.length();
		if (sublen == 0) { return startIndex > srclen ? srclen : (startIndex < -1 ? -1 : startIndex); }
		sub = sub.toLowerCase();
	...
	public static int lastIndexOfWhitespace(String src, int startIndex, int endIndex) {
		...
		for (int i = startIndex; i >= endIndex; i--) {
			if (Character.isWhitespace(src.charAt(i))) { return i; }
		}
		return -1;
	}
	...
	public static int lastIndexOfNonWhitespace(String src, int startIndex, int endIndex) {
		...
		for (int i = startIndex; i >= endIndex; i--) {
			if (Character.isWhitespace(src.charAt(i)) == false) { return i; }
		}
		return -1;
	}
	// ---------------------------------------------------------------- starts and ends
	 ...
	 * @param subS suffix
	 * @return <code>true</code> if the character sequence represented by the argument is a suffix of the character sequence represented by this object; <code>false</code> otherwise. */
	...
	public static int countIgnoreCase(String source, String sub) {
		...
		if (sublen == 0) { return 0; }
		while (true) {
			int i = StringUtil.indexOfIgnoreCase(source, sub, j);
			if (i == -1) {
				break;
			}
			count++;
	...
	// ---------------------------------------------------------------- string arrays
	/** Finds the very first index of a substring from the specified array. It returns an int[2] where int[0] represents the substring index and int[1] represents position where substring was found.
	 * Returns <code>null</code> if noting found.
	 * @param s source string
	...
	/** Finds the very last index of a substring from the specified array. It returns an int[2] where int[0] represents the substring index and int[1] represents position where substring was found.
	 * Returns <code>null</code> if noting found.
	 * @param s source string
	 * @param arr string array */
	...
	public static boolean equals(String as[], String as1[]) {
		...
		for (int i = 0; i < as.length; i++) {
			if (as[i].equals(as1[i]) == false) { return false; }
		}
		return true;
	}
	/** Compares two string arrays.
	...
	public static boolean equalsIgnoreCase(String as[], String as1[]) {
		if (as.length != as1.length) { return false; }
		for (int i = 0; i < as.length; i++) {
			if (as[i].equalsIgnoreCase(as1[i]) == false) { return false; }
		}
		return true;
	}
	...
	public static String replace(String s, String[] sub, String[] with) {
		if ((sub.length != with.length) || (sub.length == 0)) { return s; }
		int start = 0;
		StringBuilder buf = new StringBuilder(s.length());
		while (true) {
			int[] res = StringUtil.indexOf(s, sub, start);
			if (res == null) {
				break;
			}
			int end = res[1];
	...
	public static String replaceIgnoreCase(String s, String[] sub, String[] with) {
		...
		StringBuilder buf = new StringBuilder(s.length());
		while (true) {
			int[] res = StringUtil.indexOfIgnoreCase(s, sub, start);
			if (res == null) {
				break;
			}
			int end = res[1];
	...
	public static int equalsOneIgnoreCase(String src, String[] dest) {
		for (int i = 0; i < dest.length; i++) {
			if (src.equalsIgnoreCase(dest[i])) { return i; }
		}
		return -1;
	}
	...
	public static int startsWithOneIgnoreCase(String src, String[] dest) {
		for (int i = 0; i < dest.length; i++) {
			...
			if (StringUtil.startsWithIgnoreCase(src, m)) { return i; }
		}
		return -1;
	}
	...
	public static int endsWithOneIgnoreCase(String src, String[] dest) {
		for (int i = 0; i < dest.length; i++) {
			...
			if (StringUtil.endsWithIgnoreCase(src, m)) { return i; }
		}
		return -1;
	}
	// ---------------------------------------------------------------- char based
	...
	public static int indexOfWhitespace(String string, int startindex, int endindex) {
		for (int i = startindex; i < endindex; i++) {
			if (CharUtil.isWhitespace(string.charAt(i))) { return i; }
		}
		return -1;
	}
	...
	public static int indexOfNonWhitespace(String string, int startindex, int endindex) {
		for (int i = startindex; i < endindex; i++) {
			if (CharUtil.isWhitespace(string.charAt(i)) == false) { return i; }
		}
		return -1;
	}
	// ---------------------------------------------------------------- strip, trim
	...
	public static String stripLeadingChar(String string, char c) {
		if (string.length() > 0) {
			if (string.charAt(0) == c) { return string.substring(1); }
		}
		return string;
	}
	...
	public static String stripTrailingChar(String string, char c) {
		if (string.length() > 0) {
			if (string.charAt(string.length() - 1) == c) { return string.substring(0, string.length() - 1); }
		}
		return string;
	}
	 ...
	 * </ul>
	 * <p>
	 * Escape character may be used to prefix boundaries so they can be ignored. Double escaped region will be found, and first index of the result will be decreased to include one escape character.
	...
	/** Joins list of iterable elements. Separator string may be <code>null</code>. */
	public static String join(Iterable<?> elements, String separator) {
		if (elements == null) { return StringPool.EMPTY; }
	 ...
	 * <ul>
	 * <li>Every upper case letter in the CamelCase name is translated into two characters, a separator and the lower case equivalent of the target character, with three exceptions.
	 * <ol>
	 * <li>For contiguous sequences of upper case letters, characters after the first character are replaced only by their lower case equivalent, and are not preceded by a separator (
	 * <code>theFOO</code> to <code>the_foo</code>).
	 * <li>An upper case character in the first position of the CamelCase name is not preceded by a separator character, and is translated only to its lower case equivalent. (<code>Foo</code> to
	 * <code>foo</code> and not <code>_foo</code>)
	 * <li>An upper case character in the CamelCase name that is already preceded by a separator character is translated only to its lower case equivalent, and is not preceded by an additional
	 * separator. (<code>user_Name</code> to <code>user_name</code> and not <code>user__name</code>.
	 * </ol>
	 * <li>If the CamelCase name starts with a separator, then that separator is not included in the translated name, unless the CamelCase name is just one character in length, i.e., it is the
	 * separator character. This applies only to the first character of the CamelCase name. */
