...
/** <code>StringBand</code> is a faster alternative to <code>StringBuilder</code>. Instead of adding strings, they are stored in internal array. Only at the end of concatenation, when
 * <code>toString</code> is invoked, strings are joined together.
 * <p>
 * To make <code>StringBand</code> even faster, predict the number of <b>joined</b> strings (and not the final string size)! */
public class StringBand{
	...
	public StringBand() {
		array = new String[StringBand.DEFAULT_ARRAY_CAPACITY];
	}
	...
	/** Appends a character. This is <b>not</b> efficient as in <code>StringBuilder</code>, since new string is created. */
	...
	/** Appends string representation of an object. If <code>null</code>, the <code>'null'</code> string will be appended. */
	...
	public char charAt(int pos) {
		...
		for (int i = 0; i < index; i++) {
			...
			if (pos < newlen) { return array[i].charAt(pos - len); }
			len = newlen;
		}
	...
	public String toString() {
		// special cases
		if (index == 0) { return StringPool.EMPTY; }
		if (index == 1) { return array[0]; }
		if (index == 2) { return array[0].concat(array[1]); }
		if (index == 3) { return array[0].concat(array[1]).concat(array[2]); }
		// join strings
