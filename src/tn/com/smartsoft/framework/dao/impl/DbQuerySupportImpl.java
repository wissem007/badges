...
import tn.com.smartsoft.framework.dao.utils.AliasToBeanResultTransformer;
import tn.com.smartsoft.framework.dao.utils.InputQuery;
import tn.com.smartsoft.framework.dao.utils.QueryUtils;

public class DbQuerySupportImpl implements DbQuerySupport, Serializable {
	...
	public DbQuerySupportImpl(DbSession dbSession) {
		super();
	...
	public int executeUpdate(String queryString, String[] paramNames, Object[] paramValues, final DbQueryAccessor accessor) throws DaoFunctionalException {
		...
		Integer updateCount = (Integer) ((DbSessionImpl) dbSession).executeDbCallback(new DbCallback() {
			public Object doExecute(Session session) throws HibernateException {
				Query queryObject;
				queryObject = session.createQuery(inputQuery.request());
	...
	public List execute(final String queryString, final String[] paramNames, final Object[] paramValues, final Class classBeanResult) throws DaoFunctionalException {
		return this.execute(queryString, paramNames, paramValues, classBeanResult, null);
	}

	public List execute(final String queryString, final String[] paramNames, final Object[] paramValues, final Class classBeanResult, final DbQueryAccessor accessor)
			throws DaoFunctionalException {
		final Query queryObject = prepareQuery(queryString, paramNames, paramValues, classBeanResult, accessor);
	...
	private Query prepareQuery(final String queryString, final String[] paramNames, final Object[] paramValues, final Class classBeanResult,
		...
		return (Query) ((DbSessionImpl) dbSession).executeDbCallback(new DbCallback() {
			public Object doExecute(Session session) throws HibernateException {
				...
				Query queryObject;
				queryObject = session.createQuery(inputQuery.request());
				if (classBeanResult != null && !classBeanResult.getName().startsWith("java"))
					queryObject.setResultTransformer(new AliasToBeanResultTransformer(classBeanResult));
				QueryUtils.prepareQuery(queryObject, accessor);
				QueryUtils.applyParameter(queryObject, inputQuery.getValues());
				return queryObject;
			}
