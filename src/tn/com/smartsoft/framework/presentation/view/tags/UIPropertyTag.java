...
public class UIPropertyTag implements IDefinition {
	...
	public void addXsd(XsSchema xsSchema, XsComplexType xsComplexType, Object parentBean) throws UIParseException {
		try {
			...
			attXsComplexType.addAttribute(new XsAttribute(this.getAttribute(), ParserBeanAccessorUtils.getPropertyType(parentBean, this.getProperty()), required,
					getDefaultValue()));
		} catch (Exception e) {
	...
	public void parse(Object component, TagHandler tagHandler) throws UIParseException {
		try {
			String value = tagHandler.getAttributeValue(this.getAttribute());
			if (StringUtils.isBlank(value))
				value = tagHandler.getValue();
			if (StringUtils.isBlank(value))
