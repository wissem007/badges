Ext.isInstanceof=function(o1,o2){
...
(function(){
	...
	Ext.ux.Media=function(config){
		this.toString=this.asMarkup; // Inline rendering support for this and
		// all subclasses
		Ext.apply(this,config||{});
		this.initMedia();
	};
	var ux=Ext.ux.Media, stateRE=/4$/i;
	...
	Ext.ux.Media.Window=Ext.extend(Ext.Window,{
		...
		afterRender:function(){
			ux.Window.superclass.afterRender.apply(this,arguments); // wait
			// for
			// sizing
			componentAdapter.afterRender.apply(this,arguments);
		},
	...
	Ext.ux.Media.Element=Ext.extend(Ext.Element,{
		...
		remove:function(cleanse,deep){
			if(this.dom) {
				this.unmask(true);
				this.removeAllListeners(); // remove any Ext-defined
				// DOM listeners
	...
	Ext.ux.Media.mediaTypes=
			{
				WAV:Ext.apply({
					...
					classid:"CLSID:22d6f312-b0f6-11d0-94ab-0080c74c7e95" // default
					,
				...
				APPLET:{
					...
					unsupportedText:{
						tag:'p',
						html:'Java is not installed/enabled.'
					},
					params:{
						url:'@url',
...
(function(){
	...
	Ext.ux.Media.Flash=
			Ext.extend(Ext.ux.Media,{
				...
				/**
				 * @private Remove the following characters to permit Flash
				 * ExternalInterface: +-/\*.
				 */
				assertId:function(id,def){
					id||(id=def||Ext.id());
				...
				/**
				 * @private (called once by the constructor)
				 */
				initMedia:function(){
					...
					if(unsupportedText) {
						...
						unsupportedText=
								mc.unsupportedText=String.format(unsupportedText,(requiredVersion?' '+requiredVersion+' ':' '),(this.playerVersion?' '+this.playerVersion+' ':' Not installed.'));
					}
					mc.mediaType="SWF";
					if(!hasRequired) {
						this.autoMask=false;
						...
						if(canInstall&&mc.installUrl) {
							mc=mc.installDescriptor||{
								...
								// The dimensions of playerProductInstall.swf
								// must be at least 310 x 138 pixels,
								,
							...
							mc.params[this.varsName]=
									"MMredirectURL="+(mc.installRedirect||window.location)+"&MMplayerType="+(Ext.isIE?"ActiveX":"Plugin")+"&MMdoctitle="+(document.title=
											document.title.slice(0,47)+" - Flash Player Installation");
						} else {
							// Let superclass handle with unsupportedText
							// property
							mc.mediaType=null;
						}
					}
				...
				onAfterMedia:function(ct){
					...
					if(mo=this.mediaObject) {
						...
						if(Ext.isIE) {
							// fscommand bindings
							// implement a fsCommand event interface since its
							// not supported on IE when writing innerHTML
				...
				bindExternals:function(methods){
					...
					Ext.each(methods,function(method){
						...
						// Do not overwrite existing function with the
						// same name.
				...
				invoke:function(method,returnType /*
					...
					if(method&&(obj=this.getInterface())&&'CallFunction'in obj) { 
						var c=
								[String.format('<invoke name="{0}" returntype="{1}">',method,returnType),'<arguments>',(Array.prototype.slice.call(arguments,2)).map(this._toXML,this).join(''),
										'</arguments>','</invoke>'].join('');
				...
				_toXML:function(value){
					...
					} else if(Ext.isInstanceof(value,Date)) {
	...
	Ext.ux.Media.Flash.Window=Ext.extend(Ext.ux.Media.Window,{
		...
		autoScroll:false,
		/**
		 * @cfg {Boolean} shadow Set to false to prevent DOM reflow when shadow
		 * is hidden/shown
		 * @default false
		 */
		shadow:false,
	...
	Ext.ux.Media.Flash.Element=Ext.extend(Ext.ux.Media.Element,{
		remove:function(){
			...
			if(Ext.isIE&&Ext.isWindows&&(d=this.dom)) {
				...
				d.style.display='none'; // hide it regardless of
				// state
				if(d.readyState==4) {
					for ( var x in d) {
			...
			if(doc&&doc.getElementsByTagName) {
				...
				if(head&&(script=doc.createElement("script"))) {
					for ( var attrib in attributes) {
						if(attributes.hasOwnProperty(attrib)&&attribinscript) {
...
Ext
		.onReady(function(){
			...
			if(CSS&&!CSS.getRule('.x-hide-nosize')) {
				CSS
						.createStyleSheet('.x-hide-nosize{height:0px!important;width:0px!important;border:none!important;visibility:hidden!important;zoom:1;}.x-hide-nosize * {height:0px!important;width:0px!important;border:none!important;visibility:hidden!important;zoom:1;}');
				CSS.refreshCache();
			}
		});
(function(){
...
Ext.ux.IntelliMask.prototype={
	...
	show:function(msg,msgCls,fn,fnDelay){
		...
		fnDelay=parseInt(fnDelay,10)||20; // ms delay to allow mask to
		// quiesce if fn specified
		if(Ext.isObject(msg)) {
			opt=msg;
			msg=opt.msg;
			msgCls=opt.msgCls;
